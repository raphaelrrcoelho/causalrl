...
- **Local torch and pyright are broken on this machine; CI is the verifier.** Run `uv run pytest tests/<file> -v` where torch is not needed; for torch-dependent tasks, state in the commit that CI verifies.
...
**Note:** local torch is broken; CI verifies this task. Keep the test small and fixed-seed.
...
Run: `uv run pytest tests/test_scm_fitters.py -v -k neural`
Expected: PASS. If local torch is broken, state that in the commit body and rely on CI.
...
Local torch and pyright are broken on this machine, so **CI is the verifier** — nothing counts as
