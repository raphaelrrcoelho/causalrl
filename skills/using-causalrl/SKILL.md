...
| Certify a **trained policy**'s value improvement over the logging policy (any trainer) | `certify_policy(dataset, target_actions, gamma_max=…, alpha=…)`; the `alpha` gate is `conformal_action_value` (distribution-free lower bound on a fresh return) |
...
5. **The MSM sensitivity is on the propensity weights** (sharp when target supports are disjoint, valid-but-conservative otherwise). It does not certify an arbitrary outcome-model/doubly-robust point estimate.
