...
A `LinearGaussianPopulationEnv` exposes one ego agent embedded in a population as a per-agent causal
...
from causalrl import CausalGraph, certify_effect, linear_gaussian_population_env
...
def main() -> None:
    env = linear_gaussian_population_env(ego="ego", coplayer="co", ego_effect=1.5, confound=1.0)
    log = env.sample(4_000, seed=0)
    ...
    # Interventional ground truth from the environment's do().
    do1 = _floats(env.do({"ego": 1}, 20_000, seed=1).values_by_name("Y")).mean()
    do0 = _floats(env.do({"ego": 0}, 20_000, seed=2).values_by_name("Y")).mean()
