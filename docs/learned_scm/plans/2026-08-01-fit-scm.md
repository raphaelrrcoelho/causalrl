...
- **Verify locally, then in CI.** The local toolchain works (torch 2.12.0, numpy 2.4.6, `uv run pyright src` → 0 errors, pytest green — checked 2026-08-01). Every task runs its own tests locally with `uv run pytest tests/<file> -v --no-cov` before committing. CI remains the final gate, but "CI will catch it" is not an acceptable substitute for running the tests you just wrote.
- **Pyright strict** runs in CI over `src` only. Duck-typed adapters need `# type: ignore[...]` on the specific line, matching the existing style in `scm/scm.py`.
...
- [ ] **Full suite:** `uv run pytest -q` — all tests pass, including the pre-existing SCM suites that prove specified models are unaffected.
- [ ] **Types:** `uv run pyright src` (the exact CI invocation, not single-file).
- [ ] **Lint:** `uv run ruff check src tests examples && uv run ruff format --check src tests examples`
- [ ] **Push and watch CI.** CI is the verifier for `NeuralFit` and for anything torch-dependent. Do not claim green before the run passes.
