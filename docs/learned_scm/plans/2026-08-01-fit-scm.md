...
Run: `uv run pytest tests/test_scm_fitters.py -k poisson tests/test_scm_fit.py -k lag -v --no-cov`
Expected: FAIL — `ImportError: cannot import name 'PoissonGLMFit'`. The lag test should **pass already** (`@`/`-` in node names was verified working on 2026-08-02); if it fails, that is a real regression — report it rather than adjusting the test.
