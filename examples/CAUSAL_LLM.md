...
| `causal_rlvr_trace.py` | ladder §5: decoupled RLVR — GRPO from the R2 supervised state, OUTCOME vs STRUCT (oracle-verified trace F1) rewards + CONFBOOST exposure control. Instrumented negative: STRUCT saturates at 1.99/2.0 on the RL pool (policy already trace-perfect on TRAIN confounded graphs) ⇒ zero advantage ⇒ the confounded corruption is a train→fresh-instance generalization gap, unreachable by on-policy rewards; predicted lever = confounded-pattern data diversity | canonical-negative · RLVR (ladder) |
