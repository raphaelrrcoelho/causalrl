...
- **`conformal_action_value(dataset, target_actions, *, alpha)`** (`causalrl.conformal`, exported
  ...
  this computes it from any `LoggedDecisions` log and the same per-decision
  ...
  confounding, positivity (with the effective sample size and the unsupported actions as
  diagnostics, under the `"unsupported"` detail key).
...
- `InterventionSpace` gained domain objects: `values(variable)` raises `TypeError` on a continuous
  domain (an interval has no arm list) and `assignments()` propagates it, while the new
  `domain(variable)` returns the domain itself. A raw value tuple still means `Discrete`, so
  construction from 2.x is unchanged.
