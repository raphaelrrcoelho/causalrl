...
:class:`~eqcert.games.CausalGame` (a MAID with a decision and utility node per agent). A
...
from eqcert.certify.certificate import Kind
from eqcert.games import CausalGame, decision_node, utility_node
...
def topology_max_kind(topology: LearnerTopology) -> Kind:
    """The strongest epistemic :class:`~eqcert.certify.certificate.Kind` ``topology`` licenses."""
...
class Population:
    ...
    def to_game(self) -> CausalGame:
        """Materialise the normal-form :class:`~eqcert.games.CausalGame` (small ``N``).
