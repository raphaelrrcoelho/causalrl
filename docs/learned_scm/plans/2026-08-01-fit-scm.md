...
### Task 12: `PoissonGLMFit` — count mechanisms for point-process data

**Requested by** the `causalrl.neuro` consumer of this branch (2026-08-02), as P0 with P1 scoring.

**Files:**
- Modify: `src/causalrl/scm/fitters.py` (append the family)
- Modify: `tests/test_scm_fitters.py`
- Modify: `tests/test_scm_fit.py` (lag-embedded node-name regression test)

**Interfaces:**
- Consumes: `FittedMechanism`, `MechanismFitter`, `evaluate_holdout` (Tasks 3–6).
- Produces: `PoissonGLMFit(max_iter: int = 50, tol: float = 1e-8)`, reporting `invertible=False` and attaching `log_prob`.

**Why this family exists.** Binned spike counts are integer-valued with few distinct levels, so `_is_discrete` routes them to `TabularCPT`, which builds a table with one row per parent configuration. Lag-unrolled spike nodes routinely carry 6–12 lagged parents; at ~3 levels each that is 729 to 531,441 configurations, nearly all unobserved and dominated by the Laplace prior. It also discards the point-process structure: a conditional intensity is log-linear in its inputs, so a GLM is linear in the parents where the table is exponential in them.

**`invertible=False` is correct here, not a limitation.** A Poisson draw is not recoverable from `(parents, count)` — many latent draws produce the same count — so counterfactuals at this node are genuinely unidentified and the L3 guard should refuse. Task 6 already requires every non-invertible fitter to attach `log_prob`, and `evaluate_holdout` routes non-invertible families to **mean held-out log-likelihood** rather than R². So the requested Poisson-appropriate holdout score falls out of the contract with no extra mechanism.

**Not auto-dispatched.** `_is_discrete` keeps routing counts to `TabularCPT`; this family is opt-in via `fit_scm(families={"M1_0@t-0": PoissonGLMFit()})`. Changing the dtype heuristic would silently alter existing behaviour.

- [ ] **Step 1: Write the failing tests**

```python
# tests/test_scm_fitters.py (append)
...
```python
# tests/test_scm_fit.py (append) — the lag-embedded node-name guarantee
...
Run these as **two separate commands** — `-k` is a single-valued option, not per-path, so a combined invocation would silently apply only the last `-k` to both files; and a collection error in the first file aborts the whole session before the second is reached:

```bash
uv run pytest tests/test_scm_fitters.py -k poisson -v --no-cov   # expect the ImportError
uv run pytest tests/test_scm_fit.py -k lag -v --no-cov           # expect PASS
```

Expected: the first FAILS with `ImportError: cannot import name 'PoissonGLMFit'`. The second should **pass already** (`@`/`-` in node names was verified working on 2026-08-02); if it fails, that is a real regression — report it rather than adjusting the test.

- [ ] **Step 3: Implement `PoissonGLMFit`**

Append to `src/causalrl/scm/fitters.py`. Fit `log E[V | pa] = intercept + w·pa` by IRLS, then make it structural with the canonical inverse-CDF construction, exactly as `TabularCPT` does: `V = F⁻¹(U | pa)` with `U ~ Uniform(0, 1)`.
...
- IRLS: start `beta = 0`; iterate `eta = X @ beta`, `mu = exp(eta)`, working response `z = eta + (y - mu) / mu`, weights `w = mu`, solve the weighted least squares `beta = (Xᵀ W X)⁻¹ Xᵀ W z`. Stop when `‖Δbeta‖∞ < tol` or `max_iter` is hit. Clip `eta` to about `[-30, 30]` before `exp` to avoid overflow on wide starts.
- Use `np.linalg.solve` on the weighted normal equations with a small ridge (`1e-8 * I`) so a rank-deficient design fails gracefully rather than raising.
- Expose `mechanism.coefficients` as `{"intercept": float, **{parent: float}}` for the test and for diagnostics.
- The inverse CDF: for each unit, `lam = exp(eta)`; find the smallest `k` with `P(V ≤ k | lam) ≥ u`. Vectorise by accumulating the Poisson CDF up to a cap (e.g. `max(20, ceil(lam.max() + 10 * sqrt(lam.max())))`) and using the same `(u > cdf).sum(dim=1)` trick `TabularCPT` uses, clamped to the cap.
- `log_prob(parent_values, values)` returns `v * log(lam) - lam - lgamma(v + 1)` elementwise as a Tensor.
- Sort parent names (`sorted(parents)`) for determinism, matching every other family.

- [ ] **Step 4: Run the tests to verify they pass**

Run: `uv run pytest tests/test_scm_fitters.py tests/test_scm_fit.py -v --no-cov`
Expected: PASS, with no existing test disturbed.

- [ ] **Step 5: Export and document**

Add `PoissonGLMFit` to the `_LAZY` table and `__all__` in both `src/causalrl/__init__.py` and `src/causalrl/scm/__init__.py`, matching the entries Task 9 added. Extend the `test_learned_scm_surface_is_exported_top_level` name list.

- [ ] **Step 6: Commit**

```bash
git add src/causalrl/scm/fitters.py src/causalrl/__init__.py src/causalrl/scm/__init__.py \
        tests/test_scm_fitters.py tests/test_scm_fit.py tests/test_public_api.py
git commit -m "feat(scm): PoissonGLMFit -- log-linear count mechanisms for point-process data"
```

---

### Task 13: `BayesianLinearFit` — posterior mechanisms as a continuous SCM belief

**Files:**
- Create: `src/causalrl/scm/continuous/bayesian_fit.py`
- Create: `tests/test_bayesian_fit.py`
- Modify: `pyproject.toml` (coverage `omit`), `.github/workflows/ci.yml` (numpyro lane)

**Interfaces:**
- Consumes: `FittedMechanism`, `_attach_residual` (Tasks 3–4); `_require_numpyro()`'s pattern from `src/causalrl/scm/continuous/nuts.py:47`.
- Produces: `BayesianLinearFit(draws: int = 1000, warmup: int = 1000, seed: int = 0)`, reporting `invertible=True`, with `mechanism.posterior` carrying the parameter draws.

**Why.** `fit_scm` returns point-estimated mechanisms, and `fit_scm_mec` represents the SCM belief as a *discrete* list of equivalence-class members. A posterior over structural coefficients is the continuous version of the same idea, and it is the form that composes with the certificate layer: mechanism uncertainty propagates into an estimand band, which is what this library is for.

**Design: the posterior is additional structure, not a replacement.** The returned `mechanism` is the **posterior-mean** `LinearGaussianMechanism`, so every invariant the other continuous families satisfy still holds — `invertible=True`, `residual` attaches, the round-trip identity holds, and `evaluate_holdout` scores it by R² like its siblings. The draws are attached as `mechanism.posterior` (a dict of arrays keyed `"intercept"`, each parent name, and `"sigma"`). Materialising one SCM per draw is a natural follow-on; do not build it here.

**Dependency gating.** NumPyro is an existing optional extra (`causalrl[numpyro]`), absent from the main CI matrix and pinned `python_version < '3.14'`. Follow `nuts.py` exactly: raise a clear "install the `causalrl[numpyro]` extra" `ImportError` at call time, `pytest.importorskip("numpyro")` at the top of the test module, add the new module to the coverage `omit` list in `pyproject.toml` beside `*/scm/continuous/nuts.py`, and add the test file to the existing py3.11 numpyro CI lane. Without the omit entry the main lane's 90% coverage gate will fail on an uncovered module.
...
from causalrl.scm.continuous.bayesian_fit import BayesianLinearFit  # noqa: E402
...
Run: `uv run --python 3.11 --extra numpyro pytest tests/test_bayesian_fit.py -v --no-cov`
Expected: FAIL — `ModuleNotFoundError: No module named 'causalrl.scm.continuous.bayesian_fit'`

- [ ] **Step 3: Implement**

```python
# src/causalrl/scm/continuous/bayesian_fit.py
...
- Reuse `nuts.py`'s import-guard shape: a module-level `_require_numpyro()` raising `ImportError` naming the `causalrl[numpyro]` extra.
- Model: `intercept ~ Normal(0, 10)`, `w_j ~ Normal(0, 10)`, `sigma ~ HalfNormal(1)`, `y ~ Normal(intercept + X @ w, sigma)`. Standardise `X` internally and unstandardise the draws so the priors stay weakly informative regardless of scale.
- Seed NUTS with `jax.random.PRNGKey(self.seed)` so runs are reproducible, matching the determinism rule every other family follows.
- Build the returned mechanism from the posterior **means**: `LinearGaussianMechanism(names, weights, bias=intercept_mean)`, noise `Normal(0, sigma_mean)`, then `_attach_residual(mechanism, names, mean_fn)` with the same intercept-column `mean_fn` `LinearGaussianFit` uses. Attach `mechanism.posterior` and `mechanism.coefficients`.
- `score` is `_r2` of the posterior-mean prediction against the training data, so the number is comparable with the other continuous families.

- [ ] **Step 4: Run the test to verify it passes**

Run: `uv run --python 3.11 --extra numpyro pytest tests/test_bayesian_fit.py -v --no-cov`
Expected: PASS (3 tests). NUTS is slow — if the suite exceeds roughly a minute, lower `draws`/`warmup` in the tests rather than loosening a tolerance.

- [ ] **Step 5: Wire the coverage omit and the CI lane**

In `pyproject.toml`, add `"*/scm/continuous/bayesian_fit.py"` to `[tool.coverage.run] omit`, beside the existing `nuts.py` entry, with a comment matching its rationale.

In `.github/workflows/ci.yml`, extend the numpyro lane's test step:

```yaml
      - name: NUTS abduction tests
        run: uv run --python 3.11 --no-sync pytest tests/test_scm_nuts.py tests/test_bayesian_fit.py -v --no-cov
```

- [ ] **Step 6: Verify the main lane is unaffected**

Run: `uv run pytest tests/test_bayesian_fit.py -v --no-cov` (without the extra)
Expected: SKIPPED, not errored — `importorskip` must fire cleanly.

Then `uv run pytest --cov-fail-under=90` to confirm the coverage gate still passes with the new module omitted.

- [ ] **Step 7: Commit**

```bash
git add src/causalrl/scm/continuous/bayesian_fit.py tests/test_bayesian_fit.py \
        pyproject.toml .github/workflows/ci.yml
git commit -m "feat(scm): BayesianLinearFit -- posterior structural coefficients via NUTS"
```

---

### Task 14: Lean pass — remove overengineering and overtesting

**Run last**, after Tasks 11–13, so it sweeps everything the sub-project accumulated. The `/simplify` skill covers exactly this shape of work and is the natural way to execute it.

**Files:** every file this plan created or modified. No new files.

**Goal:** the smallest library that does what the design says, with tests that all earn their place. No behaviour changes.

#### The one rule that governs test removal

A test earns its place if **breaking the code it names makes it fail**. That is the criterion — not line count, not coverage percentage, not how obvious it looks.

This matters here more than usual. Four separate tests on this branch were rewritten specifically because they asserted an outcome the correct code produces without checking that broken code produces a different one, and each took multiple review rounds to fix. **Do not delete a test without first mutation-testing it**: break the behaviour it names, confirm it fails, restore. If it fails, it stays — no matter how redundant it looks. If it passes while the code is broken, it is noise and should go regardless of how thorough it looks.

Specifically protected, all mutation-verified already: the `residual` round-trip tests (Task 4), the tier-conflict cycle-path assertion (Task 1), the two Fréchet-floor discriminating tests (Task 8), the `holdout_score` scoring-method test (Task 6), the `invertible`-wiring consumer test (Task 6), and the v-structure exclusion test (Task 7). Leave these alone.

#### Delete: dead code and unused surface

- [ ] `_abduct(..., allow_fitted=...)` in `src/causalrl/scm/scm.py` — the parameter is never read in the body and has **zero callers**; `counterfactual_interval` inverts noise directly rather than abducting. Remove the parameter. `_abduct` being private and unguarded is self-evident from its name. If a later sub-project needs a flag, it can add one meaningfully then.
- [ ] `_config_index`'s `if not parent_names:` guard in `src/causalrl/scm/fitters.py` — unreachable; `TabularCPT.fit` short-circuits before ever calling it with empty parents.
- [ ] `_Empirical.stddev` in `src/causalrl/scm/fitters.py` — defined, never called anywhere in the library or tests.
- [ ] The `with torch.no_grad():` wrapper around `predicted` in `NeuralFit.fit` — redundant now that parameters are frozen immediately above it.
- [ ] `sorted(...)` computed twice in two raises — `sorted(ambiguous)` in `scm.py`'s guard, `sorted(upstream)` in `counterfactual_bounds.py`. Bind once, use twice.

#### De-duplicate

- [ ] The parent-columns-to-numpy conversion appears in `_attach_residual`'s `residual` closure and again in `ANMFit.fit`'s `mechanism` closure in `src/causalrl/scm/fitters.py`. Extract one helper. Judge first whether it is genuinely the same operation — if the two differ in dtype handling or shape, leave them and say so.

#### Fix, don't test around

- [ ] `TabularCPT.alpha` is unvalidated. With `alpha <= 0` and a parent configuration that has no observations, `counts.sum()` is zero, the table becomes NaN, and NaN comparisons silently route every draw to index 0 rather than raising. A two-line `__init__` guard raising on `alpha <= 0` is leaner and safer than a regression test for the NaN path. Do not add the test; add the guard.

#### Consciously decline

The deferred-minor list from the review rounds also contains *additions*. A lean pass should decline them unless the untested path is load-bearing. Record the decision rather than silently skipping:

- `fit_scm_mec`'s cyclic-orientation branch is untested (all fixtures are tree-shaped skeletons). The mechanism it delegates to — `CausalGraph.__init__`'s DAG check — is already well covered. **Decline**, note in `PROGRESS.md`.
- No test uses a non-default `TabularCPT.alpha`, a CPDAG with non-empty `directed_edges`, `RealizabilityError`, multi-node interventions, or a >2-level discrete target through the interval path. The `>2`-level exactness of `_extreme_under_sum` was verified pre-implementation against an LP over the full transportation polytope, which is stronger evidence than a unit test would be. **Decline all**, note them as known coverage boundaries.

#### Re-examine, then decide

- [ ] `CounterfactualBound.tight` is `True` on every path phase 1 can reach, because the hard case raises instead of returning a loose bound. Judge honestly whether it is forward-compatible design or speculative generality. The argument for keeping it: sub-project 2 returns valid-but-loose bounds, and adding a field to a released NamedTuple later is a breaking change. The argument for cutting it: nothing needs it today. **Recommend keeping** — the whole reason it is a separate type from `Interval` is that `Interval` could not absorb a third field without breaking tuple unpacking, and that lesson is fresh. But state the call explicitly in the report.
- [ ] `# type: ignore[code]` is line-blanket in pyright, not rule-scoped — only `# pyright: ignore[code]` scopes to a rule. The codebase uses the former in ~15 places. **Do not churn the existing ones**; this is a project-wide convention question, not a lean-pass decision. Note it and move on.

#### Constraints

- **No behaviour changes.** Every one of the ~890 tests must still pass, untouched except for any this task deletes under the mutation rule.
- Coverage must not drop below the 90% gate. Deleting dead code raises coverage; deleting a test lowers it. If a deletion drops you near the line, that is a signal the code being tested is not actually dead.
- Full CI gate before committing: `uv run ruff check .`, `uv run ruff format --check .`, `uv run pyright src`, `uv run pytest --cov-fail-under=90`. Read each output.
- Report every deletion with its mutation evidence, and every decline with its reason.

- [ ] **Commit**

```bash
git commit -m "refactor(scm): lean pass -- drop dead surface, de-duplicate, guard TabularCPT.alpha"
```

---

### Task 15: RL audit — is any of this reinforcement learning?

**Run last**, after Task 14. Requested by the user on 2026-08-03: *"is this reinforcement learning? sometimes I feel we deviate too much from RL."*

**The concern, stated plainly.** The library is called `causalrl`. This sub-project added six ways to fit a mechanism from a static table of numbers. Fitting a Poisson GLM to a fixed dataset is supervised learning; identifying an interventional distribution is causal inference. **Neither is reinforcement learning.** There is no policy in this sub-project, no reward, no environment, no rollout, no regret, no exploration, no value function. The user's instinct is very likely correct, and the audit should be prepared to confirm it rather than argue it away.

**But the library does have a real RL spine already** — `envs/` (the Gymnasium `CausalEnvWrapper`, `register_envs`, the SCB bandit suite), `agents/` (MABUC, `CausalMBRLAgent`, `GFormulaBackdoorAgent`), the CGFA-PPO `factored_advantage` primitive, off-policy evaluation, `certify_policy`, and the d3rlpy bridge. So the question is not "should this library do RL" — it does. The question is whether **this sub-project connects to that spine or grows beside it.**

**The correction is connection, never deletion.** "Without losing features" is a hard constraint: every fitter stays. A learned SCM is exactly what model-based RL calls a *world model*, and the library already has the agent that would use one. If the wiring is missing, add the wiring.

**Method:** answer each question with *executable evidence*, not opinion. Cite `file.py:line`, and where you claim something works, show it running.

- [ ] **Q1 — Inventory: where is the RL surface, and does `fit_scm` touch any of it?**

  List the concrete RL entry points the library ships today (environments, agents, policy evaluation, regret/bandit machinery) with file:line. Then grep for any reference to `fit_scm`, `FitReport`, or `provenance="fitted"` from inside `envs/` or `agents/`. Report the count honestly — if it is zero, say zero. That number *is* the answer to the user's question, and it should lead the report.

- [ ] **Q2 — Can a fitted SCM be an environment you act in?**

  This is the cheapest real RL connection and it may already work by construction: `CausalEnvWrapper` and `envs/base.py:19` take a `StructuralCausalModel`, and `fit_scm` returns one. **Verify it executes end-to-end** — fit an SCM from data, hand it to the env surface, step/rollout it, apply an intervention, and confirm the interventional rollout differs from the observational one in the direction the fitted mechanism implies. If it works, that is the headline finding and it deserves a test, not just a paragraph. If it breaks, the break is a real bug: find out whether it is the fitted model or the wrapper, and fix the smaller side.

- [ ] **Q3 — Can a fitted SCM serve as the world model for `CausalMBRLAgent`?**

  `CausalMBRLAgent` is the library's discover → plan → certify → transport agent, and its whole premise is a causal model it plans in. Trace whether it can consume a `fit_scm` result, or whether it builds its model by a separate internal path that `fit_scm` cannot feed. Two outcomes, both worth reporting: (i) the seam exists — demonstrate it and test it; (ii) the seam does not exist — that is sub-project 3 (`plan-in-model`), deliberately sequenced later. **Do not build sub-project 3 here.** Record precisely what is missing so the gap is a known boundary rather than an oversight.

- [ ] **Q4 — Does a fitted model reach *policy*-level surfaces, not just estimand-level ones?**

  The distinction that matters for the user's question: `E[Y | do(A=1)]` is an estimand; `V(π)` for a policy `π` that maps states to actions is RL. Check whether a fitted SCM can be handed to `certify_policy`, the OPE surfaces, or `PolicyValueContrast`. If a fitted world model can evaluate a policy, this sub-project is doing RL work; if it can only produce a single interventional mean, it is doing causal inference that RL could later use.

- [ ] **Q5 — The examples are the visible answer. Do any of them contain a policy?**

  `examples/learned_scm_nhefs.py` estimates an average treatment effect. `examples/learned_scm_oracle_gate.py` compares `E[W | do(Z=1)]` against an oracle. Both are estimand-level; neither has an agent, an action chosen over time, or a reward. Whatever the internal wiring says, **this is what a reader sees**, and it reads as causal inference, not RL.

  If Q2 shows the env path works, add **one** example that closes the loop end-to-end: learn the SCM from logged data → use it as a world model → choose actions with it → evaluate the resulting policy against a baseline. Keep it small and honest, and be explicit about the regime where this is supposed to win (confounded / offline / transfer) rather than claiming a general RL improvement — the project's own benchmarking already established that causal point estimates do not reliably beat strong contenders on real data. One worked example that genuinely runs beats three paragraphs of framing.

- [ ] **Q6 — Naming and framing.**

  Check `README.md`, `docs/tour.md`, the top-level `__init__.py` docstring, and `docs/learned_scm/DESIGN.md`. Is `fit_scm` presented as *learning the world model an agent acts in*, or as a standalone modelling feature? Content stays; framing changes. If the honest framing is "this is the model-learning half of model-based RL, and the acting half is sub-project 3," write exactly that — do not oversell the connection.

- [ ] **Q7 — The verdict, in the user's terms.**

  End `docs/learned_scm/PROGRESS.md` with a direct answer to "is this reinforcement learning?" Acceptable answers include "no, and here is why it still belongs in this library," "partly — the env path works, the planning path does not," and "yes, via X." What is not acceptable is redefining RL until the answer comes out yes. Then state whether the remaining sub-project order (4 → 3 → 2: NCM, then plan-in-model, then active discovery) still makes sense, given that **sub-project 3 is the RL bridge** and the user has now twice signalled that the RL connection matters. Make a recommendation; the resequencing decision is the user's.

**Constraints**

- **No feature loss.** Every public name that exists after Task 14 still exists after Task 15, with the same behaviour.
- New code is allowed only for wiring and the one example in Q5 — plus a genuine bug fix if Q2 or Q3 uncovers one. No new subsystem.
- Full CI gate before committing: `uv run ruff check .`, `uv run ruff format --check .`, `uv run pyright src`, `uv run pytest --cov-fail-under=90`.
- Report each question with its evidence, each change made, and each gap consciously left open.

- [ ] **Commit**

```bash
git commit -m "feat(scm): connect the learned SCM to the RL surface -- world model, env rollout, worked example"
```
